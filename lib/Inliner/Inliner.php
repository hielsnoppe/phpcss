...
/**
 * This class is based on Pelago\Emogrifier
 * @see https://github.com/jjriv/emogrifier/blob/V1.0.0/Classes/Emogrifier.php
 *
 * Many changes have been made, though, including:
 * - CSS selectors are handled by the Symfony\CssSelector component
 *   This reduces some (rarely used) pseudo-selector capabilities, but this also
 *   reduces complexity inside this class. With respect to separation of
 *   concerns, the lost functionality should be implemented inside the
...
class Inliner {
    ...
    /**
     * @var Symfony\Component\CssSelector\CssSelectorConverter $selectorConverter
     */
    protected $selectorConverter = null;
    ...
    protected function process(\DOMDocument $xmlDocument)
    {
        ...
        foreach ($cssRules as $cssRule) {
            ...
            $nodesMatchingCssSelectors = $xPath->query($this->selectorConverter->toXPath($cssRule['selector']));
    ...
    private function saveLinks($xpath) {

        $nodesWithHrefAttributes = $xpath->query('//@href | //@src');
        $this->links = array();
        $id = 0;

        if ($nodesWithHrefAttributes !== FALSE) {
            foreach ($nodesWithHrefAttributes as $node) {
                $this->links[] = $node->value;
                $node->value = 'nl2go_stored_link_'.$id++;
            }
        }
    ...
    private function restoreLinks ($html) {

        for ($i=(count($this->links) - 1); $i >= 0 ; $i--) {
           $html = str_replace('nl2go_stored_link_'.$i, $this->links[$i], $html);
        }
    ...
    {
        $nodesMatchingSelector = $xPath->query($this->selectorConverter->toXPath($cssSelector));
    ...
    private function getNodesToExclude(\DOMXPath $xPath)
    {
        ...
        foreach (array_keys($this->excludedSelectors) as $selectorToExclude) {
            foreach ($xPath->query($this->selectorConverter->toXPath($selectorToExclude)) as $node) {
                $excludedNodes[] = $node;
            }
        }
